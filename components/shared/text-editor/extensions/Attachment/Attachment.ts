...
import { getFileTypeIcon } from './components/NodeViewAttachment/FileIcon'
import { normalizeFileSize } from '@/utils/file'
import { getDatasetAttribute } from '@/utils/dom-dataset'
import { NodeViewAttachment } from './components/NodeViewAttachment/NodeViewAttachment'
...
export const Attachment = /* @__PURE__ */ Node.create<AttachmentOptions>({
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    const { url = '', fileName = '', fileSize = '', fileType = '', fileExt = '' } = HTMLAttributes || {}
    ...
    return [
      ...
      url
        ? [
            ...
            ['span', { class: 'attachment__text' }, `${fileName}.${fileExt} (${normalizeFileSize(fileSize)})`]
          ]
